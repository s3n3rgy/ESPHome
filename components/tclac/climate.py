...
SUPPORTED_FAN_MODES_OPTIONS = {
    "AUTO": ClimateMode.CLIMATE_FAN_AUTO,  # Default
...
SUPPORTED_SWING_MODES_OPTIONS = {
    "OFF": ClimateSwingMode.CLIMATE_SWING_OFF,  # Default
...
SUPPORTED_CLIMATE_MODES_OPTIONS = {
    "OFF": ClimateMode.CLIMATE_MODE_OFF,  # Default
    "AUTO": ClimateMode.CLIMATE_MODE_AUTO,  # Default
...
SUPPORTED_CLIMATE_PRESETS_OPTIONS = {
    "NONE": ClimatePreset.CLIMATE_PRESET_NONE, # Default
...
# Checking the interface configuration and accepting default values
    ...
    if CONF_VISUAL in config:
        ...
        if CONF_MIN_TEMPERATURE in visual_config:
            ...
            if min_temp < TCLAC_MIN_TEMPERATURE:
                raise cv.Invalid(f"The specified interface minimum temperature in {min_temp} below permissible {TCLAC_MIN_TEMPERATURE} for air conditioner")
        else:
        ...
        if CONF_MAX_TEMPERATURE in visual_config:
            ...
            if max_temp > TCLAC_MAX_TEMPERATURE:
                raise cv.Invalid(f"The specified interface maximum temperature in {max_temp} above permissible {TCLAC_MAX_TEMPERATURE} for air conditioner")
        else:
        ...
        if CONF_TEMPERATURE_STEP in visual_config:
            ...
            if ((int)(temp_step * 2)) / 2 != temp_step:
                raise cv.Invalid(f"Specified temperature step {temp_step} not correct, must be a multiple of 1")
        else:
...
# Check the component configuration and accept default values
...
# Registering events of turning on and off the air conditioner display
...
# Registration of events of turning on and off the air conditioner buzzer
@automation.register_action(
...
# Registration of events of switching on and off of module communication LEDs
@automation.register_action(
...
# Logging events for enabling and disabling forced application of settings
@automation.register_action(
...
# Registering the event of setting the vertical lock of the flap
@automation.register_action(
...
# Registration of the event of setting the horizontal fixation of the flaps
@automation.register_action(
...
# Registering the vertical swing curtain setting event
@automation.register_action(
...
# Registering the event of setting the horizontal swing of the curtains
@automation.register_action(
